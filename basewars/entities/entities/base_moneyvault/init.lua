...
function ENT:Upgrade(ply)
	...
	if (not ply:CanAfford(priceToUpgrade)) then
		return false, "Can't afford to upgrade ("..BaseWars.MonetFormat(priceToUpgrade)..")"
	end
...
function ENT:ValidateRaid(attacker)
	if not self.NoARR and IsValid(self.Owner) then
...
function ENT:DeployMoney(attacker)
	...
	if (attacker:IsPlayer()) then
		...
		if (attacker == self.Owner) then
			...
			local payMessage = Format("Paid £%s back for destroying your %s", pay, self.PrintName)
		...
		elseif (aTeam == 1 or #team.GetPlayers(aTeam) <= 1) then
			-- 70% of the total value (70% paid so far)
			local pay	= value * .7
			...
			if (owner and IsValid(owner)) then
				...
				-- 20% of the total value (90% paid total, 10% lost 'in the explosion', less efficient than faction raiding)
				local back	= value * .2
		...
		-- They are part of a faction (This is the fancy shit)
			...
			-- 75% of the total value (75% paid out so far)
			local pay	= value * .75
			...
			if (owner and IsValid(owner)) then
				...
				-- 20% of the total value (95% paid total, 5% lost 'in the explosion')
				local back	= value * .2
