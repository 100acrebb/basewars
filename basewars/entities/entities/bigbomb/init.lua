...
function ENT:Explode()
	if not BaseWars.ONGOINGRAID then 
		Notify(self.Owner,1,3, "Not in a raid, AIN'T HAVIN IT.")
	...
	local lgc1, lgc2, lgc3, lgc4
	local o = self.Owner
	lgc1 = (self.Owner:Team() == BaseWars.ONGOINGRAID[1])
	lgc2 = (self.Owner == BaseWars.ONGOINGRAID[1])
	lgc3 = (self.Owner:Team() == BaseWars.ONGOINGRAID[2])
	lgc4 = (self.Owner == BaseWars.ONGOINGRAID[2])
	
	if not (lgc1 or lgc2 or lgc3 or lgc4) then
		Notify(self.Owner,1,3, "Not in a raid, AIN'T HAVIN IT.")
		self.Entity:EmitSound(Sound("weapons/c4/c4_disarm.wav"))
		self.Entity:GetPhysicsObject():EnableMotion(true)
		self.Armed = false
	...
	for k, v in ipairs(props) do 
		...
		if (class:find("prop_physics") || class=="gmod_wheel" || class=="gmod_thruster" || class=="gmod_button" || class=="sent_keypad" || class=="auto_turret") then
			v:Remove()
		end
	end
	...
	for i=0, 15, 1 do
		...
		local randpos = Vector(math.random(-20,20), math.random(-20,20), math.random(3,30))
		bomblet.Owner = self.Owner
		bomblet:SetPos(self.Entity:GetPos()+randpos)
		bomblet:Spawn()
		bomblet:Activate()
		bomblet:GetPhysicsObject():SetVelocity(randpos*68)
		util.ScreenShake( self.Entity:GetPos(), 100, 2, 2, 10000 )
	end 
...
function ENT:Armbomb(planter)
	...
	if (self.Entity:CheckPropFaggotry()==false) then
		...
		for k, v in pairs(player.GetAll()) do
			...
			if (propnum==nil) then propnum = 0 end
			if (tonumber(propnum)>0) then
				Notify(v, 1, 3, "A bigbomb has been planted near " .. tostring(owners[uid]) .. " of your props!")
			end
		end
		timer.Create( tostring(self.Entity) .. "checkpropshit", 5, 18, function() if IsValid(self) then self:PropCheck() end end)
		timer.Create( tostring(self.Entity) .. "checkblast", 89, 1, function() if IsValid(self) then self:PropCheck() end end)
		timer.Create( tostring(self.Entity) .. "checkblast", 119, 1, function() if IsValid(self) then self:PropCheck() end end)
		timer.Create( tostring(self.Entity) .. "goboom", 120, 1, function() if IsValid(self) then self:Explode() end end)
	...
	for k, v in pairs(ents.FindInSphere( self.Entity:GetPos(), 1536) ) do
		if (player.Owner!=false) then
			...
			if self.Owner==entowner then
				...
				if v:GetClass() == "prop_physics" then propwallingbitch = true end
			end
		end
	end
