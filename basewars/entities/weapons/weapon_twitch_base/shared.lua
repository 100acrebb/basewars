...
SWEP.PrintName				= "Twitch Weapon Base"
SWEP.Category				= "Twitch Weaponry"
...
if SERVER then
	...
	hook.Add ("Think", "Twitch_SATHINK", StationaryAimingThink)
end
...
hook.Add ("RenderScreenspaceEffects", "Twitch_Effects", DrawTWEffects)
...
hook.Add ("HUDShouldDraw", "Twitch_HUD_Disabler", HUDShouldDraw)
...
hook.Add ("CalcView", "Twitch_SACV", StationaryAimingCalcView)
...
local function StationaryAimingCreateMove(cmd)
	if WeaponUsesStationaryAiming() then
		...
		LocalPlayer():SetFOV(LocalPlayer():GetActiveWeapon().FOVToSet or 90, .1)
	else
		if lockedViewAng then
			lockedViewAng = false
			lastRealViewAng = false
			LocalPlayer():SetFOV(90, .1)
		end
	end
end

hook.Add("CreateMove", "Twitch_SACM", StationaryAimingCreateMove)
